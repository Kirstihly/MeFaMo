...
def calculate_rotation(face_landmarks, pcf: PCF, image_shape):
    ...
    landmarks = np.array([(lm.x, lm.y, lm.z) for lm in face_landmarks.landmark[:468]])
...
class Mefamo:
    def __init__(
        ...
        self.live_link_face = PyLiveLinkFace(
            name="Argos", fps=30, filter_size=self.filter_size
        )
    ...
    def start(self):
        ...
        if isinstance(self.input, str) and (
            self.input.lower().endswith(".jpg") or self.input.lower().endswith(".png")
        ):
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                (
                    ...
                    rotation_vector,
                    translation_vector,
                ) = calculate_rotation(face_landmarks, self.pcf, image.shape)
        ...
        if self.show_image:
            ...
            if self.show_debug:
                for shape in FaceBlendShape:
                    ...
                    cv2.putText(
                        img=white_bg,
                        text=shape_debug_text,
                        org=tuple(text_coordinates),
                        fontFace=font,
